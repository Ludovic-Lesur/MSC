...
void AT_DecodeRxBuffer(void) {
	...
	if (at_ctx.at_rx_buf_idx < AT_COMMAND_MIN_SIZE) {
		...
		// ADC command AT$ADC?<CR>.
			...
			unsigned int solar_voltage_mv = 0;
			unsigned int output_voltage_mv = 0;
			unsigned int output_current_ua = 0;
			unsigned int mcu_voltage_mv = 0;
			signed char mcu_temperature_degrees = 0;
			// Perform ADC convertions.
			ADC1_PerformMeasurements();
			ADC1_GetSolarVoltage(&solar_voltage_mv);
			...
			USART2_SendString("mV Vout=");
			USART2_SendValue(output_voltage_mv, USART_FORMAT_DECIMAL, 0);
			USART2_SendString("mV Iout=");
			USART2_SendValue(output_current_ua, USART_FORMAT_DECIMAL, 0);
			USART2_SendString("uA Vmcu=");
			USART2_SendValue(mcu_voltage_mv, USART_FORMAT_DECIMAL, 0);
			USART2_SendString("mV Tmcu=");
